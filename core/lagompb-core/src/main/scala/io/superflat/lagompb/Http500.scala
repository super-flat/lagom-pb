 ...
 */
final class Http500(exceptionMessage: ExceptionMessage, cause: Throwable)
...
object Http500 {
  ...
  def apply(message: String) =
    new Http500(new ExceptionMessage(classOf[Http500].getSimpleName, message), new Throwable)

  def apply(cause: Throwable) =
    new Http500(new ExceptionMessage(classOf[Http500].getSimpleName, cause.getMessage), cause)
  // $COVERAGE-ON$
}
