...
class LagompbKafkaSerde extends StrictMessageSerializer[KafkaEvent] {
  private final val serializer: NegotiatedSerializer[KafkaEvent, ByteString] = {
    (serviceEvent: KafkaEvent) =>
      {
  ...
  private final val deserializer
    : NegotiatedDeserializer[KafkaEvent, ByteString] = { (bytes: ByteString) =>
