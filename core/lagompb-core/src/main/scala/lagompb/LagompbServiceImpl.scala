...
sealed trait LagompbServiceImplComponent {
  ...
  implicit val timeout: Timeout = Timeout(
    LagompbCommon.config.getInt("lagompb.ask-timeout").seconds
  )
  ...
  /**
    ...
    */
  def sendCommand[TCommand <: scalapb.GeneratedMessage,
                  TState <: scalapb.GeneratedMessage](
  ...
  private[lagompb] def parseAny[TState <: scalapb.GeneratedMessage](
    data: Any
  ): TState = {
  ...
  private[lagompb] def handleLagompbCommandReply[
    TState <: scalapb.GeneratedMessage
  ](commandReply: CommandReply): LagompbState[TState] = {
    commandReply.reply match {
      ...
      case _ =>
        throw new LagompbException(
          s"unknown LagompbCommandReply ${commandReply.reply.getClass.getName}"
        )
    }
  }
...
sealed abstract class LagompbRestServiceImpl(
  ...
  /**
    ...
    */
  final def sendCommand[TCommand <: GeneratedMessage,
                        TState <: scalapb.GeneratedMessage](
    cmd: TCommand,
    ...
    cmd.companion.scalaDescriptor.fields
      .find(
        field =>
          field.getOptions.extension(ExtensionsProto.command).exists(_.entityId)
      )
      .fold[Future[LagompbState[TState]]](
        Future.failed(BadRequest("command does not have entity key set."))
      )(fd => {
...
abstract class LagompbServiceImplWithKafka(
  ...
  aggregate: LagompbAggregate[_]
    ...
    extends LagompbRestServiceImpl(clusterSharding, persistentEntityRegistry)(
      ec
    )
  ...
  final override def kafkaEvents: Topic[KafkaEvent] =
    TopicProducer.taggedStreamWithOffset(LagompbEvent.Tag) {
      (tag, fromOffset) =>
        persistentEntityRegistry
  ...
  private def transformEvent(
    ev: EventStreamElement[LagompbEvent]
  ): KafkaEvent = {
    ev.event match {
      case EventWrapper(
          Some(anyEvent: Any),
              ...
              comp.scalaDescriptor.fields
                ...
                .map(fd => {
                  KafkaEvent()
                    .withEvent(anyEvent)
                    .withState(
                      StateWrapper().withMeta(metaData).withState(state)
                    )
                    .withPartitionKey(
                      comp
          ...
          case Success(result: Option[KafkaEvent]) =>
            result match {
              case Some(value: KafkaEvent) =>
                log.debug(
                  "[Lagompb]: event has been persisted into kafka successfully"
                )
                value
              case None =>
                throw new LagompbException(
                  s"unable to transform event ${anyEvent.typeUrl} for kafka"
                )
            }
        }
      case _ =>
        throw new LagompbException(
          s"unknown event received ${ev.event.getClass.getName}"
        )
    }
  }
}
...
trait LagompbGrpcServiceImpl extends LagompbServiceImplComponent {
  ...
  /**
    ...
    */
  final override def sendCommand[TCommand <: GeneratedMessage,
                                 TState <: scalapb.GeneratedMessage](
    clusterSharding: ClusterSharding,
    entityId: String,
    cmd: TCommand,
