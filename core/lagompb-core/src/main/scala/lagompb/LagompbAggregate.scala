...
abstract class LagompbAggregate[TState <: scalapb.GeneratedMessage](
  ...
  final def create(entityContext: EntityContext[LagompbCommand]): Behavior[LagompbCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        AkkaTaggerAdapter.fromLagom(entityContext, LagompbEvent.Tag)
      )
      .withRetention(
        RetentionCriteria
          .snapshotEvery(
            numberOfEvents = config.getInt("lagompb.snaphsot-criteria.frequency"), // snapshotFrequency
            keepNSnapshots = config.getInt("lagompb.snaphsot-criteria.retention") //snapshotRetention
          )
      )
  }
    ...
    Try {
      ...
      case Success(state) =>
        ...
        commandHandler.handle(cmd, state, stateWrapper.getMeta) match {
                  ...
                  case Event(event: Any) =>
                    LagompbProtosCompanions
                      ...
                      .fold[ReplyEffect[LagompbEvent, StateWrapper]](
                        ...
                        val resultingState: TState = eventHandler.handle(event.unpack(comp), state, eventMeta)
          ...
          case Failure(exception) =>
            val errMsg: String = s"command handler breakdown, ${exception.getMessage}"
            log.error(errMsg, exception)
            throw exception
        }
    }
