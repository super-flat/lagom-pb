 ...
 */
abstract class LagompbSlickPgRepository[T](tableQuery: TableQuery[T], database: Database) {
   ...
   */
  def read(entityId: String): Future[Option[T]]
   ...
   */
  def update(model: T): Future[Option[T]]
   ...
   */
  def delete(entityId: String): Future[Option[T]]
