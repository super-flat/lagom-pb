...
object LagompbCommon {
  ...
  def loadFileObjects(): Seq[GeneratedFileObject] = {
    ...
    fileObjects.foldLeft(Seq.empty[GeneratedFileObject])((seq, fo) => {
      Try {
        val runtimeMirror: universe.Mirror = universe.runtimeMirror(fo.getClassLoader)
        val module: universe.ModuleSymbol = runtimeMirror.staticModule(fo.getName)
        ...
        case Success(fileObject) =>
          val subMsg: String = fileObject.messagesCompanions
            ...
            .mkString("")
          val msg: String = s"|\t - fileObject jvmName: ${fileObject.getClass.getCanonicalName}"
