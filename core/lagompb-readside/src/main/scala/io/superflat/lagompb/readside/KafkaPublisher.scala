...
abstract class KafkaPublisher(encryptionAdapter: EncryptionAdapter)(
  ...
  private[this] val sendProducer: SendProducer[String, String] = SendProducer(
    ProducerSettings(actorSystem, new StringSerializer, new StringSerializer)
      .withBootstrapServers(producerConfig.bootstrapServers))(actorSystem.toClassic)

  private def producerRecord(
      event: Any,
      state: Any,
      meta: MetaData,
      partitionKey: String): ProducerRecord[String, String] = {
    new ProducerRecord(
